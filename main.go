...
var (
        ...
        SmsSignName = GetConfig.GetValue("sms", "signname")
        SmsTemplate = GetConfig.GetValue("sms", "template")
        SmsAppKey = GetConfig.GetValue("sms", "appkey")
        SmsAppSecret = GetConfig.GetValue("sms", "appsecret")
...
func main() {
        ...
        e.GET("/auth", WxAuth)
        e.POST("/send", SendMsg)
        e.POST("/sendsms", SendSMS)

        //port := GetConfig.GetValue("http", "port")
        bind := GetConfig.GetValue("http", "bind")
        if bind == "no value" {
                e.Logger.Fatal(e.Start("127.0.0.1:4567"))
        } else {
...
func SendSMS(context echo.Context) error {
        ...
        alidayu.AppKey = SmsAppKey
        alidayu.AppSecret = SmsAppSecret
        alidayu.UseHTTP = true // set UseHTTP to true if prefer http over https
        result, resp := alidayu.SendSMS(toUser, SmsSignName, SmsTemplate, `{"server":"` + content + `"}`)

        log.Printf("发短信给%s, 内容: %s, 结果: %v, resp: %s", toUser, content, result, resp)
        return context.String(200, string(resp))
}
